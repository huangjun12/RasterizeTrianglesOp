...
def test_rasterize_triangles(self):
    image_height = 4
    image_width = 4
    num_triangles = 20
    v_data = (2 * np.random.rand(12 * num_triangles) - 1).reshape(3 * num_triangles, 4).astype('float32')
    t_data = (np.array([i for i in range(3 * num_triangles)])).reshape(num_triangles, 3).astype('int32')

    with self.static_graph():
        vertices = layers.data(name='vertices', shape=[-1, 4], dtype='float32')
        triangles = layers.data(name='triangles', shape=[-1, 3], dtype='int32')
        barycentric_coordinates, triangle_ids, z_buffer = layers.rasterize_triangles(
            vertices, triangles, image_height=image_height, image_width=image_width)

        static_ret = self.get_static_graph_result(
            feed={'vertices':v_data,
                  'triangles':t_data},
            fetch_list=[barycentric_coordinates, triangle_ids, z_buffer])

    self.assertTrue(np.allclose(static_ret[0], static_ret[1]))
    self.assertTrue(np.allclose(static_ret[0], static_ret[2]))
